...
from PIL import Image  #0922

PATCH_SIZE = 21

# open the camera image
image = data.camera() 
im = Image.open("road_10.jpg").convert('L')       #0922
#im = data.imread("road_10.jpg")
 
# select some patches from grassy areas of the image
grass_locations = [(474, 291), (440, 433), (466, 18), (462, 236)]
grass_patches = []
for loc in grass_locations:
    grass_patches.append(im[loc[0]:loc[0] + PATCH_SIZE,
...
for loc in sky_locations:
    sky_patches.append(im[loc[0]:loc[0] + PATCH_SIZE,
                             loc[1]:loc[1] + PATCH_SIZE])
...
# for each patch, plot (dissimilarity, correlation) 畫點在correlation上
